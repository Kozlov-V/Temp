...
handle_call({login, {existing_state, State}}, _From, 
    #state{mod=Mod} = State) ->
    %Result = Mod:event(Event, Args),
    Result = Mod:login({existing_state, State}),
    {reply, Result, State};
...
%% @end
%%---------------------------------------------------------------------
login({existing_state, State}) ->
    gen_server:call(?MODULE, {login, {existing_state, State}}).
